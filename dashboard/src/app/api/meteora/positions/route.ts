...
export async function GET(req: NextRequest) {
  ...
  if (!pool || !user) return NextResponse.json({ error: 'pool and user required' }, { status: 400 })
  ...
  const res = await fetch(`${BASE}/positions/${encodeURIComponent(pool)}/pnl?${params}`, { cache: 'no-store' })

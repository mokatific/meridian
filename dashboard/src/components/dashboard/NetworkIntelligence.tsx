...
function Stat({ label, value, sub, color }: { label: string; value: string; sub?: string; color?: string }) {
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-900/60 p-3">
      ...
      <div className={cn('mt-1 text-xl font-bold tabular-nums', color ?? 'text-zinc-100')}>{value}</div>
...
function OverviewGrid({ ov }: { ov: NetworkOverview }) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
      <Stat label="Active agents" value={ov.activeAgents.toLocaleString()} sub={`of ${ov.registeredAgents.toLocaleString()} registered`} />
      <Stat label="Win rate (adj.)" value={`${ov.adjustedWinRatePct}%`}
        color={ov.adjustedWinRatePct >= 60 ? 'text-green-400' : ov.adjustedWinRatePct >= 50 ? 'text-amber-400' : 'text-red-400'} />
      <Stat label="Total fees earned" value={fmtUsd(ov.totalFeesUsd)}
        sub={`${ov.syncedWalletFeesSol.toFixed(1)} SOL synced`} color="text-green-400" />
      <Stat label="Network PnL" value={fmtUsd(ov.totalPnlUsd)}
        color={ov.totalPnlUsd >= 0 ? 'text-green-400' : 'text-red-400'} />
      <Stat label="Total closes" value={ov.sampleCount.toLocaleString()} sub={`${ov.adjustedSampleCount.toLocaleString()} adjusted`} />
      <Stat label="Avg hold time" value={`${ov.avgHoldMinutes.toFixed(1)}m`} />
      <Stat label="Avg PnL per trade" value={`${ov.avgPnlPct >= 0 ? '+' : ''}${ov.avgPnlPct.toFixed(2)}%`}
    ...
    <div className="space-y-1">
      ...
      <ResponsiveContainer width="100%" height={180}>
        <BarChart data={chartData} margin={{ left: 0, right: 8 }}>
          ...
          <YAxis yAxisId="pnl" tick={{ fill: '#71717a', fontSize: 11 }} tickFormatter={v => `$${v}`} width={50} />
          <YAxis yAxisId="closes" orientation="right" tick={{ fill: '#71717a', fontSize: 11 }} width={40} />
          <Tooltip
          ...
          <Bar yAxisId="pnl" dataKey="pnlUsd" name="pnl $"
            fill="#3b82f6" opacity={0.8}
            // color each bar red/green by value
          />
          <Line yAxisId="closes" type="monotone" dataKey="closes" name="closes" stroke="#a78bfa" strokeWidth={2} dot={false} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  )
...
function TerminalLine({ event }: { event: TerminalEvent }) {
  ...
  return (
    <div className={cn(
      ...
      event.type === 'lesson' ? 'bg-blue-950/10' : isPositive ? 'bg-green-950/10' : isNegative ? 'bg-red-950/10' : '',
    )}>
      <span className="text-zinc-700 shrink-0 w-16">{time}</span>
      <span className={cn(
        'shrink-0 w-6 font-bold',
        event.type === 'lesson' ? 'text-blue-400' : isPositive ? 'text-green-400' : isNegative ? 'text-red-400' : 'text-zinc-500',
      )}>
        {event.type === 'lesson' ? '💡' : isPositive ? '▲' : isNegative ? '▼' : '•'}
      </span>
      <span className="text-amber-400/80 shrink-0">{agentId}</span>
      <span className={cn('shrink-0', event.type === 'lesson' ? 'text-blue-400' : 'text-zinc-500')}>{verb}</span>
  ...
  return (
    <div className="rounded-lg border border-zinc-800 overflow-hidden">
      ...
      <div ref={scrollRef} className="max-h-80 overflow-y-auto">
        {events.map((e, i) => <TerminalLine key={i} event={e} />)}
      </div>
    </div>
  )
}
        ...
        <tbody>
          {pools.map(p => (
            <tr key={p.pool} className="border-b border-zinc-800/40 hover:bg-zinc-800/20">
              ...
              <td className="py-2 px-2 text-right text-zinc-500">{p.adjustedSampleCount.toLocaleString()}</td>
              <td className={cn('py-2 px-2 text-right font-medium', p.totalPnlUsd >= 0 ? 'text-green-400' : 'text-red-400')}>{fmtUsd(p.totalPnlUsd)}</td>
              <td className="py-2 px-2 text-right text-zinc-300">{fmtUsd(p.totalFeesUsd)}</td>
              <td className={cn('py-2 px-2 text-right', p.adjustedWinRatePct >= 65 ? 'text-green-400' : p.adjustedWinRatePct >= 55 ? 'text-amber-400' : 'text-zinc-400')}>
                {p.adjustedWinRatePct}%
              </td>
              <td className={cn('py-2 px-2 text-right', p.avgPnlPct >= 0 ? 'text-green-400' : 'text-red-400')}>
                {p.avgPnlPct >= 0 ? '+' : ''}{p.avgPnlPct.toFixed(2)}%
              </td>
              <td className="py-2 px-2 text-right text-zinc-500">{p.avgHoldMinutes.toFixed(0)}m</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  if (isLoading) {
    return (
      <div className="space-y-3">
        <div className="grid grid-cols-4 gap-2">{Array.from({ length: 8 }).map((_, i) => <Skeleton key={i} className="h-16 bg-zinc-900" />)}</div>

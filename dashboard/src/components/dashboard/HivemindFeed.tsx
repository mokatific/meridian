...
function ConsensusBadge({ consensus, contradictory }: { consensus: string; contradictory: boolean }) {
  if (contradictory)
    return <Badge variant="destructive" className="text-xs">contradictory</Badge>
  if (consensus === 'strong')
            ...
            <Card key={lesson.id} className="border-zinc-800 bg-zinc-900/60">
              <CardContent className="p-4 space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <ConsensusBadge consensus={lesson.consensus} contradictory={lesson.contradictory} />
                  <span className="text-xs text-zinc-500">{lesson.distinctAgents} agents · {lesson.sampleCount} samples · conf {(lesson.confidence * 100).toFixed(0)}%</span>
                  <span className="ml-auto text-xs text-zinc-600">{new Date(lesson.created_at).toLocaleDateString()}</span>
                </div>

...
import { createCachedConnection, sendAndConfirmPolling } from "../utils/rpc-cache.js";
import {
  trackPosition,
  markOutOfRange,
  markInRange,
...
function getTransactionInstructions(tx) {
  ...
  return tx.message.compiledInstructions
    .map((ix) => {
      ...
      const accounts = indexes.map((accountIndex) => keys[accountIndex]).filter(Boolean);
  ...
  for (const [index, serialized] of (serializedTxs || []).entries()) {
    ...
    for (const account of requiredStaticAccounts.filter(Boolean)) {
      if (!staticKeys.includes(String(account))) {
        throw new Error(
          `Relay ${label || "transaction"} ${index + 1} missing required account ${String(account).slice(0, 8)}.`,
        );
      }
    }
    ...
    if (value.err) {
      throw new Error(
        `Relay ${label || "transaction"} ${index + 1} simulation failed: ${JSON.stringify(value.err)}`,
      );
    }

    if (
      ownerIndex >= 0 &&
      value.preBalances?.[ownerIndex] != null &&
      value.postBalances?.[ownerIndex] != null
    ) {
    ...
    for (const [mint, preAmount] of preByMint) {
      if (preAmount > 0n && !allowedMints.has(mint)) {
        throw new Error(
          `Relay ${label || "transaction"} ${index + 1} would close/debit unrelated token mint ${mint}.`,
        );
      }
    }
...
async function _deployPositionInner({
  ...
  let activeBinsBelow =
    bins_below ?? config.strategy.defaultBinsBelow ?? config.strategy.minBinsBelow;
  let activeBinsAbove = bins_above ?? 0;
  const parsedVolatility = volatility == null ? null : Number(volatility);
  const normalizedVolatility =
    parsedVolatility != null && Number.isFinite(parsedVolatility) ? parsedVolatility : null;

  if (volatility != null && (normalizedVolatility == null || normalizedVolatility <= 0)) {
    throw new Error(
      `Invalid volatility ${volatility} — refusing deploy because the volatility feed is unusable.`,
    );
  }
  ...
  if (
    !Number.isFinite(finalAmountY) ||
    !Number.isFinite(finalAmountX) ||
    finalAmountY < 0 ||
    finalAmountX < 0
  ) {
    throw new Error(
  ...
  const minBinsBelow = Math.max(
    MIN_SAFE_BINS_BELOW,
    Number(config.strategy.minBinsBelow ?? MIN_SAFE_BINS_BELOW),
  );
  ...
  if (process.env.DRY_RUN === "true") {
    return {
      ...
      message:
        "DRY RUN — no transaction sent. Deploy recorded as successful for simulation purposes. Do NOT call deploy_position again.",
    };
  }
  ...
  if (shouldUseLpAgentRelayForDeploy()) {
    try {
      ...
      if (addLiquidityUnsigned.length + swapUnsigned.length === 0) {
        throw new Error(
          "LPAgent order returned no transactions. Check the pool address, deploy amount, and selected range.",
        );
      }
  ...
  try {
    ...
    if (isWideRange) {
      ...
      for (let i = 0; i < createTxArray.length; i++) {
        ...
        const txHash = await sendAndConfirmPolling(getConnection(), createTxArray[i], signers);
      ...
      for (let i = 0; i < addTxArray.length; i++) {
        const txHash = await sendAndConfirmPolling(getConnection(), addTxArray[i], [wallet]);
        txHashes.push(txHash);
      ...
      const txHash = await sendAndConfirmPolling(getConnection(), tx, [wallet, newPosition]);
      txHashes.push(txHash);
    }
...
async function fetchLpAgentOpenPositions(walletAddress) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      log(
        "lpagent_api",
        `HTTP ${res.status} for owner ${walletAddress.slice(0, 8)}: ${body.slice(0, 160)}`,
      );
      return {};
    }
    const data = await res.json();
...
export async function getPositionPnl({ pool_address, position_address }) {
  ...
  if (shouldUseLpAgentRelay()) {
    try {
      ...
      log(
        "pnl_warn",
        "Relay positions API did not include requested position; falling back to Meteora PnL path",
      );
    } catch (error) {
      log(
        "pnl_warn",
  ...
  try {
    ...
    const unclaimedValue = solMode
      ? safeNum(p.unrealizedPnl?.unclaimedFeeTokenX?.amountSol) +
        safeNum(p.unrealizedPnl?.unclaimedFeeTokenY?.amountSol)
    ...
    return {
      ...
      all_time_fees_usd: roundNum(
        solMode ? p.allTimeFees?.total?.sol : p.allTimeFees?.total?.usd,
        4,
      ),
  ...
  const loadPositions = async () => {
    try {
      ...
      if (shouldUseLpAgentRelay()) {
        try {
          if (!silent)
            log("positions", "Fetching raw LPAgent open positions via Agent Meridian relay...");
...
export async function claimFees({ position_address }) {
  ...
  try {
    ...
    const txHashes = [];
    for (const tx of txs) {
      const txHash = await sendAndConfirmPolling(getConnection(), tx, [wallet]);
      txHashes.push(txHash);
    }
...
export async function closePosition({ position_address, reason }) {
  ...
  try {
    ...
    if (shouldUseLpAgentRelay()) {
      ...
      try {
        ...
        if (closeUnsigned.length + swapUnsigned.length === 0) {
          throw new Error(
            "LPAgent close order returned no transactions. Check the position, selected output, and relay order response.",
          );
        }
        ...
        if (!closedConfirmed) {
          return {
            success: false,
            error:
              "Close submit succeeded but position still appears open after verification window",
            position: position_address,
            pool: poolAddress,
            ...
            for (let attempt = 0; attempt < 6; attempt++) {
              ...
              if (res.ok) {
                ...
                if (posEntry) {
                  ...
                  pnlUsd = config.management.solMode
        ...
        log(
          "close_warn",
          `Relay zap-out failed before submit; falling back to local close + Jupiter autoswap: ${relayError.message}`,
        );
      }
    }
      ...
      if (recentlyClaimed) {
        ...
        if (claimTxs && claimTxs.length > 0) {
          for (const tx of claimTxs) {
            const claimHash = await sendAndConfirmPolling(getConnection(), tx, [wallet]);
    ...
    if (hasLiquidity) {
      ...
      const txHash = await sendAndConfirmPolling(getConnection(), closeTx, [wallet]);
      closeTxHashes.push(txHash);
    }
    const txHashes = [...claimTxHashes, ...closeTxHashes];
    ...
    if (tracked) {
          ...
          if (res.ok) {
            ...
            if (posEntry) {
              ...
              if (shouldRejectClosedPnl(nextPnlPct, reason || tracked?.close_reason)) {
                ...
                log(
                  "close",
                  `Closed PnL from API: pnl=${pnlUsd.toFixed(2)} ${config.management.solMode ? "SOL" : "USD"} (${pnlPct.toFixed(2)}%), withdrawn=${finalValueUsd.toFixed(2)} USD, deposited=${initialUsd.toFixed(2)} USD`,
                );
                break;
              }
            } else {
      ...
      if (finalValueUsd === 0) {
        ...
        if (cachedPos) {
          pnlTrueUsd =
            cachedPos.pnl_true_usd ?? (config.management.solMode ? 0 : cachedPos.pnl_usd) ?? 0;

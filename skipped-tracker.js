...
function load() {
  if (!fs.existsSync(SKIPPED_FILE))
    return {
      entries: [],
      stats: { total_skipped: 0, total_evaluated: 0, missed_opportunities: 0 },
    };
  try {
...
export function recordSkippedPools(filteredPools, totalScreened) {
  ...
  log(
    "skipped-tracker",
    `Recorded ${filteredPools.length} skipped pools (total tracked: ${data.entries.length})`,
  );
}
...
export async function evaluateSkippedPools() {
  ...
  log(
    "skipped-tracker",
    `Evaluated ${batch.length} skipped pools: ${results.length} missed opportunities found`,
  );
...
async function deriveMissedOpportunityLessons(missedPools) {
  ...
  for (const missed of missedPools) {
    const rule =
      `MISSED OPPORTUNITY: ${missed.pool_name} was skipped (reason: ${missed.skip_reason}) but generated ~${missed.estimated_fee_yield_pct}% fee yield in ${missed.metrics_at_skip?.hours_since || "3"}h. ` +
      `Metrics at skip: ${formatMetrics(missed.metrics_at_skip)}. Consider relaxing the filter that caused this rejection.`;

    addLesson(
      rule,
      ["missed_opportunity", "threshold_review", categorizeReason(missed.skip_reason)],
      {
        role: "SCREENER",
      },
    );
    ...
    log(
      "skipped-tracker",
      `Missed opportunity lesson: ${missed.pool_name} (${missed.skip_reason}) → ${missed.estimated_fee_yield_pct}% yield`,
    );
  }
}
...
export function getSkippedStatsForPrompt() {
  ...
  lines.push(
    `  Evaluated: ${stats.total_evaluated} | Missed: ${stats.missed_opportunities} (${stats.total_evaluated > 0 ? Math.round((stats.missed_opportunities / stats.total_evaluated) * 100) : 0}% miss rate)`,
  );
  ...
  if (stats.by_reason.length > 0) {
    ...
    for (const r of stats.by_reason.slice(0, 5)) {
      ...
      lines.push(
        `    ${r.reason}: ${r.missed_opportunities}/${r.total_skipped} missed (${r.miss_rate_pct}%)`,
      );
    }
  }

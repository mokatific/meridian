...
positions.command(["wallet", "status"], async (ctx) => {
  try {
    ...
    const suffix =
      ctx.message.text.startsWith("/status") && pos.total_positions
...
positions.hears(/^\/pool\s+(\d+)$/i, async (ctx) => {
  try {
    ...
    if (idx < 0 || idx >= pos.length) {
      await ctx.reply("Invalid number. Use /positions first.");
      return;
    }
    const p = pos[idx];
    await ctx.reply(
  ...
  try {
    ...
    for (const p of pos) {
      try {
        const result = await closePosition({ position_address: p.position });
        results.push(
          `${p.pair}: ${result.success ? "closed" : `failed (${result.error || "unknown"})`}`,
        );
      } catch (error) {

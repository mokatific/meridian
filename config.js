...
const configuredMaxBinsBelow =
  numericConfig(u.maxBinsBelow) ??
  (legacyBinsBelow != null ? Math.max(legacyBinsBelow, configuredMinBinsBelow) : 69);
const configuredDefaultBinsBelow =
  numericConfig(u.defaultBinsBelow) ?? legacyBinsBelow ?? configuredMaxBinsBelow;
...
export const config = {
  ...
  screening: {
    ...
    tokenAgeSweetMinHours: u.tokenAgeSweetMinHours ?? 12,
    tokenAgeSweetMaxHours: u.tokenAgeSweetMaxHours ?? 48,
    surfaceTokenAge: u.preset === "main" && u.surfaceTokenAge !== false,
  ...
  jupiter: {
    ...
    referralAccount:
      process.env.JUPITER_REFERRAL_ACCOUNT ?? "9MzhDUnq3KxecyPzvhguQMMPbooXQ3VAoCMPDnoijwey",
...
export function reloadScreeningThresholds() {
  try {
    ...
    const maxBinsBelow =
      numericConfig(fresh.maxBinsBelow) ??
      numericConfig(fresh.binsBelow) ??
      config.strategy.maxBinsBelow;
    const defaultBinsBelow =
      numericConfig(fresh.defaultBinsBelow) ??
      numericConfig(fresh.binsBelow) ??

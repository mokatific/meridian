...
export async function evolveSmartWallets(poolAddresses = [], candidatePools = []) {
  ...
  if (result.added.length || result.removed.length) {
    log(
      "wallet_evo",
      `Evolution complete — added: ${result.added.length}, removed: ${result.removed.length}, total: ${totalAfter}`,
    );
  }
...
async function discoverLpWallets(poolAddresses) {
  ...
  for (const poolAddr of poolAddresses.slice(0, 3)) {
    try {
      const { data } = await getTopLPers(poolAddr, {
        limit: 20,
        order_by: "win_rate",
        sort_order: "desc",
      });
      for (const entry of data) {
        ...
        const pnlPct =
          entry.total_inflow > 0 ? ((entry.total_pnl ?? 0) / entry.total_inflow) * 100 : 0;
  ...
  for (const [address, data] of discovered) {
    ...
    const effectiveWinRate = data.total_positions >= 5 ? data.win_rate : data.win_rate * 0.85; // 15% discount for thin data
    ...
    if (lpCount >= MAX_LP_WALLETS) {
      log(
        "wallet_evo",
        `LP wallet list full (${MAX_LP_WALLETS}) — skipping ${address.slice(0, 8)}`,
      );
      break;
    }
    ...
    if (addResult.success) {
      log(
        "wallet_evo",
        `Added ${name} — win_rate=${(data.win_rate * 100).toFixed(0)}%, eff_win_rate=${(effectiveWinRate * 100).toFixed(0)}%, positions=${totalPositions}, avg_pnl=${data.avg_pnl_pct.toFixed(1)}%`,
      );
    ...
    try {
      ...
      for (const hc of holderCandidates) {
        ...
        if (holderCount >= MAX_HOLDER_WALLETS) {
          log(
            "wallet_evo",
            `Holder wallet list full (${MAX_HOLDER_WALLETS}) — skipping ${hc.address.slice(0, 8)}`,
          );
          break;
        }
      ...
      log(
        "wallet_evo",
        `Holder discovery failed for ${poolName || baseMint.slice(0, 8)}: ${err.message}`,
      );
    }
  }
...
function _updateWalletStats(address, newData) {
  try {
    ...
    stats.avg_pnl_pct = _lerp(
      stats.avg_pnl_pct ?? newData.avg_pnl_pct,
      newData.avg_pnl_pct,
      weight,
    );
    ...
    stats.effective_win_rate = totalP >= 5 ? stats.win_rate : (stats.win_rate ?? 1) * 0.85;
    stats.last_seen = new Date().toISOString();
    wallet.stats = stats;

    fs.writeFileSync(WALLETS_PATH, JSON.stringify(data, null, 2));
  } catch {
    /* non-critical — don't break screening */
  }
}
...
export function feedbackToWallets(walletAddresses, outcome) {
  ...
  try {
    ...
    for (const addr of walletAddresses) {
      ...
      stats.effective_win_rate = newPositions >= 5 ? stats.win_rate : stats.win_rate * 0.85;
      ...
      log(
        "wallet_evo",
        `Feedback: ${wallet.name} — pnl=${outcome.pnl_pct?.toFixed(1)}%, new win_rate=${(stats.win_rate * 100).toFixed(0)}%, positions=${newPositions}`,
      );
    }

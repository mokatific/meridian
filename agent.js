...
export async function agentLoop(
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    try {
      ...
      let toolChoice =
        step === 0 && agentType !== "SCREENER" && (ACTION_INTENTS.test(goal) || mustUseRealTool)
          ? "required"
          : "auto";
      ...
      for (let attempt = 0; attempt < 4; attempt++) {
        ...
        if (errCode === 429) {
          ...
          log(
            "agent",
            `Rate limited (429 in response), backing off ${backoff / 1000}s (attempt ${attempt + 1}/4)`,
          );
          await new Promise((r) => setTimeout(r, backoff));
          ...
          if (attempt === 1 && usedModel !== FALLBACK_MODEL) {
            ...
            log(
              "agent",
              `Provider error ${errCode}, retrying in ${wait / 1000}s (attempt ${attempt + 1}/4)`,
            );
      ...
      if (!response.choices?.length) {
        ...
        if (_lastRawJson?.choices?.length) {
          log(
            "agent",
            `SDK parse failed but raw response has ${_lastRawJson.choices.length} choice(s) — using raw`,
          );
      ...
      if (!msg.content && msg.reasoning_content) {
        log(
          "agent",
          `Content empty but reasoning present — promoting reasoning_content (${msg.reasoning_content.length} chars) to content`,
        );
        msg.content = msg.reasoning_content;
      }
      ...
      if (!msg.tool_calls || msg.tool_calls.length === 0) {
        ...
        if (!msg.content) {
          ...
          if (emptyStreak >= 3) {
            log(
              "error",
              `Empty response streak (${emptyStreak}) — aborting to prevent infinite loop`,
            );
            return {
              content:
                "The model returned empty responses repeatedly. This usually means the reasoning budget was exhausted. Try again later or switch to a different model.",
              userMessage: goal,
            };
          }
          ...
          log(
            "agent",
            `Empty response (streak ${emptyStreak}/3), retrying after ${backoff / 1000}s...`,
          );
          await new Promise((r) => setTimeout(r, backoff));
          continue;
        }
        ...
        if (mustUseRealTool && !sawToolCall) {
          ...
          log(
            "agent",
            `Rejected no-tool final answer (${noToolRetryCount}/1) for tool-required request`,
          );

...
if today_closes:
    ...
    print(
        f"Today win rate: {len(win)}/{len(today_closes)} ({100 * len(win) / len(today_closes):.0f}%)"
    )
...
gap_lessons = [
    ...
    if "gap" in l.get("rule", "").lower()
    or "skip" in l.get("rule", "").lower()
    or "miss" in l.get("rule", "").lower()
]
if gap_lessons:
...
if today_tight:
    ...
    for p in today_tight:
        ...
        print(
            f"    • {p.get('pool_name', '?')}: {p.get('final_pnl_pct', 0):.1f}% | {reason[:60]}... | held {p.get('minutes_held', 0)}min | fees=${p.get('fees_earned_usd', 0):.2f}"
        )
        ...
        if "peak" in reason.lower() and "drop" in reason.lower():
            try:
                ...
                drop = float(drop_str)
                print(
                    f"      peak={peak}% → drop={drop}pp → retention={(peak - drop) / peak * 100:.0f}%"
                )
            except:
                pass
...
daily_stats = defaultdict(
    lambda: {"count": 0, "total_fees": 0, "total_pnl": 0, "total_held": 0}
)
...
for day in sorted(daily_stats.keys()):
    ...
    print(
        f"  {day}: {d['count']} closes | avgPnL={avg_pnl:.1f}% | fees=${d['total_fees']:.2f} | avgHold={avg_held:.0f}min{marker}"
    )
...
for p in today_closes:
    ...
    fee_rate = (
        p.get("fees_earned_usd", 0) / (init_val * hours_held) * 100
        if hours_held > 0
        else 0
    )
    pnl_rate = p.get("final_pnl_pct", 0) / hours_held if hours_held > 0 else 0
    print(
        f"    {p.get('pool_name', '?'):15s}: fees=${p.get('fees_earned_usd', 0):.2f} | {p.get('final_pnl_pct', 0):.1f}% | {hours_held:.1f}h | feeYield={fee_rate:.3f}%/hr | pnlRate={pnl_rate:.1f}%/hr"
    )
...
if sq is not None:
    ...
    evtypes = c.fetchall()
    print(
        f"  Journal events: {sum(r['cnt'] for r in evtypes)} total ({len(evtypes)} types)"
    )
    for r in evtypes:
    ...
    if mgmt_days:
        ...
        for r in mgmt_days:
            print(
                f"    {r['day']}: {r['cycles']} cycles | {r['llmCycles']} LLM actions | avgPos={r['avgPositions']} | avgVal=${r['avgValue']:.2f}"
            )
    ...
    if os.path.exists(bl_path):
        ...
        if blacklist:
            ...
            for mint, info in sorted(blacklist.items()):
                print(
                    f"    • {info.get('symbol', '?')} ({mint[:10]}...{mint[-6:]}): {info.get('reason', '')[:60]}"
                )
        else:
    ...
    if bottoms:
        ...
        for r in bottoms:
            print(
                f"    • {r['pool_name']}: avgPnl={r['avgPnl']}% | avgHeld={r['avgMin']:.0f}m | closes={r['cnt']}"
            )
...
low_organic_fails = [
    p
    for p in closed
    if p.get("organic_score", 100) < 70 and p.get("final_pnl_pct", 10) < 3
]
if low_organic_fails:
    ...
    for p in low_organic_fails:
        print(
            f"      • {p.get('pool_name', '?')}: organic={p.get('organic_score', 0)} PnL={p.get('final_pnl_pct', 0):.1f}%"
        )
...
if high_vol:
    ...
    for p in high_vol:
        print(
            f"      • {p.get('pool_name', '?')}: vol={p.get('volatility', 0):.2f} PnL={p.get('final_pnl_pct', 0):.1f}% held={p.get('minutes_held', 0)}min"
        )
...
if tp_hits:
    ...
    for p in tp_hits:
        print(
            f"      • {p.get('pool_name', '?')}: {p.get('final_pnl_pct', 0):.1f}% | ${p.get('fees_earned_usd', 0):.2f} fees | held {p.get('minutes_held', 0)}min"
        )
...
config_opt_lessons = [
    l for l in lesson_entries if l.get("sourceType") == "config_change"
]

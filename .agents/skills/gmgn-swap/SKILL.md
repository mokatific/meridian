...
gmgn-cli swap \
  ...
  --sell-ratio-type hold_amount < wallet_address > --input-token \
  < token_A_address > --amount
```
...
gmgn-cli swap \
  ...
  --sell-ratio-type buy_amount < wallet_address > --input-token \
  < token_A_address > --amount
```
...
gmgn-cli multi-swap \
  ...
  --slippage 0.01 < addr1 > , < addr2 > --input-token < input_token_address > --output-token < output_token_address > --input-amount

# Sell a percentage of each wallet's balance (use --input-amount-bps)
gmgn-cli multi-swap \
  --chain sol \
  --accounts \
  '{"<addr1>":"5000","<addr2>":"10000"}' \
  --slippage 0.01 < addr1 > , < addr2 > --input-token < token_address > --output-token < sol_address > --input-amount-bps
...
gmgn-cli multi-swap \
  ...
  --tip-fee 0.00001 \
  --condition-orders '[{"order_type":"profit_stop","side":"sell","price_scale":"100","sell_ratio":"100"},{"order_type":"loss_stop","side":"sell","price_scale":"50","sell_ratio":"100"}]' < addr1 > , < addr2 > --input-token \
  < token_address > --input-amount
...
gmgn-cli multi-swap \
  ...
  --output-token \
  '{"<0xaddr1>":"1000000","<0xaddr2>":"2000000"}' \
  --slippage 0.01 \
  --gas-price 5 < 0xaddr1 > , < 0xaddr2 > --input-token \
  < token_address > --input-amount
```
...
gmgn-cli order quote \
  --chain sol \
  --from \
  0.01 < wallet_address > --input-token < input_token_address > --output-token < output_token_address > --amount < input_amount_smallest_unit > --slippage
```
...
gmgn-cli order strategy create \
  ...
  --check-price 0.002 \
  --amount-in 1000000 \
  --slippage 0.01 < wallet_address > --base-token < token_address > --quote-token < sol_address > --order-type

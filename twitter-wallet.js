...
const DEFAULT_KOLS = ["EvilPanda", "arip13741167", "4thinfected"];
...
export async function discoverWalletsFromKolTweets({
  ...
  for (const [addr, info] of addressSet) {
    log(
      "twitter_wallet",
      `  ${addr.slice(0, 12)}... → from @${info.sourceKol} (${info.sourceType})`,
    );
  }
  ...
  for (const [address, info] of addressSet) {
    ...
    for (const hc of holderCandidates) {
      ...
      log(
        "twitter_wallet",
        `  → Candidate: ${name} (${hc.address.slice(0, 8)}..., trend=${hc.cluster_trend})`,
      );
    }
  }
  ...
  log(
    "twitter_wallet",
    `Discovery complete: ${candidates.length} candidate(s) from ${addressSet.size} address(es)`,
  );
  return {

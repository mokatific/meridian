...
export function runCausalAnalysis(perfData) {
  ...
  if (feeBuckets.length >= 2) {
    ...
    if (best.win_rate > 0.6 && best.sample_count >= MIN_SAMPLES_PER_BUCKET) {
      const minFee = best.label.includes("very_high")
        ? 0.5
        : best.label.includes("high")
          ? 0.3
          : 0.1;
      recommendations.push({
        type: "screening",
  ...
  if (rangeBuckets.length >= 2) {
    ...
    if (poor && poor.sample_count >= MIN_SAMPLES_PER_BUCKET && poor.win_rate < 0.3) {
      recommendations.push({
        ...
        action:
          "Increase bins_below (maxBinsBelow) to widen range coverage. Consider raising minBinsBelow.",
  ...
  if (organicBuckets.length >= 2) {
    ...
    const best = organicBuckets.reduce(
      (b, f) => (f.win_rate > b.win_rate ? f : b),
      organicBuckets[0],
    );
    const worst = organicBuckets.reduce(
      (w, f) => (f.win_rate < w.win_rate ? f : w),
      organicBuckets[0],
    );
    if (best.win_rate - worst.win_rate > STRONG_EFFECT_THRESHOLD) {
      const minOrganic = best.label.includes("very_high")
        ? 85
        : best.label.includes("high")
          ? 75
          : 65;
  ...
  log(
    "causal",
    `Analysis: ${findings.length} findings, ${recommendations.length} recs, ${lessonsAdded} lessons added`,
  );
...
function analyzeBoolean(perfData, predicate, withLabel, withoutLabel) {
  ...
  if (withGroup.length < MIN_SAMPLES_PER_BUCKET || withoutGroup.length < MIN_SAMPLES_PER_BUCKET)
    return null;
...
function analyzeHoldDuration(perfData) {
  ...
  const results = buckets
    .map((b) => {
      const g = perfData.filter(
        (p) => p.minutes_held != null && p.minutes_held >= b.min && p.minutes_held < b.max,
      );

...
for p in today_deployed:
    ...
    if p.get("closed_at"):
        try:
            ...
            pass
    print(
        f"  {p['pool_name']} closed={p['closed']} reason={p.get('close_reason', 'open')}{dur} fees=${p.get('fees_earned_usd', 0)} pnl={p.get('final_pnl_pct', 0)}%"
    )
...
for p in positions:
    if p.get("closed"):
        try:
            ...
            if mins < 30:
                print(
                    f"FAST CLOSE: {p['pool_name']} {mins}min reason={p.get('close_reason', '?')}"
                )
...
print()
today_closes = [
    p for p in positions if p.get("closed") and p.get("closed_at", "").startswith(today)
]
low_yield_today = [
    p for p in today_closes if "low_yield" in (p.get("close_reason") or "").lower()
]
...
for p in today_closes:
    print(
        f"  {p['pool_name']}: {p.get('close_reason', '?')} (held {p.get('minutes_held', '?')}min, fees=${p.get('fees_earned_usd', 0)})"
    )

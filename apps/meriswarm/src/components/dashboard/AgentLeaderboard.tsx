  ...
  return (
    <div className="mt-2 space-y-3 rounded-lg border border-zinc-700 bg-zinc-900/80 p-3">
      <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-500">
        ...
        <span>·</span>
        <span>
          win rate <span className={winRateColor(agent.winRatePct)}>{agent.winRatePct}%</span>
        </span>
        <span>·</span>
        ...
        {walletData && (
          <div className="rounded border border-zinc-800 bg-zinc-800/50 px-2 py-1.5 text-xs">
            ...
            <span className="ml-3 text-zinc-600">
              {walletData.recentSignatures.length} recent txns
            </span>
            <a
              href={`https://solscan.io/account/${walletData.wallet}`}
              target="_blank"
              rel="noreferrer"
            ...
            <a
              href={`https://birdeye.so/profile/${walletData.wallet}?chain=solana`}
              target="_blank"
              rel="noreferrer"
      ...
      {agentEvents.length > 0 && (
        <div>
          <div className="text-xs text-zinc-600 mb-1">
            Recent activity ({agentEvents.length} events)
          </div>
              ...
              <div key={l.id} className="rounded border border-zinc-800 bg-zinc-900/60 p-2 text-xs">
                ...
                {l.tags && l.tags.length > 0 && (
                  <div className="mt-1 flex flex-wrap gap-1">
                    {l.tags.map((t) => (
                      <span key={t} className="rounded bg-zinc-800 px-1 text-zinc-500">
                        {t}
                      </span>
                    ))}
                  </div>
...
export function AgentLeaderboard() {
  ...
  const agents: GraphAgent[] = useMemo(() => {
    ...
    return [...data.graph.agents]
      ...
      .sort((a, b) =>
        sortBy === "winRate" ? b.winRatePct - a.winRatePct : a.id.localeCompare(b.id),
      );
  }, [data, search, sortBy]);
    ...
    return (
      <div className="space-y-2">
        {Array.from({ length: 10 }).map((_, i) => (
          <Skeleton key={i} className="h-9 bg-zinc-900" />
        ))}
      </div>
    );
  }
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <span className="text-xs text-zinc-600">
          {agents.length} of {data.graph.agents.length} agents
        </span>
      </div>
        ...
        <table className="min-w-full text-xs">
          ...
          <tbody>
            {agents.map((agent, i) => {
              ...
              const recentCount = terminalFeed.filter((e) =>
                e.line.startsWith(agent.id + " "),
              ).length;
                ...
                <>
                  ...
                  >
                    ...
                    <td className="py-2.5 px-2 text-right text-zinc-600">
                      {recentCount > 0 ? `${recentCount} events` : "—"}
                    </td>
                  </tr>
                  {isExpanded && (

...
function StrategyCards({ tagStats }: { tagStats: TagStat[] }) {
  const strategies = STRATEGY_TAGS.map((t) => tagStats.find((s) => s.tag === t)).filter(
    Boolean,
  ) as TagStat[];
      ...
      {strategies.map((s) => (
        <div key={s.tag} className="rounded-lg border border-zinc-800 bg-zinc-900/60 p-4">
          ...
          <div className="text-xs text-zinc-600">
            {s.good}W / {s.bad}L · {s.total} lessons
          </div>
  ...
  return (
    <tr className="border-b border-zinc-800/40 hover:bg-zinc-800/20 text-xs">
      <td className="py-1.5 px-2 font-mono text-zinc-500">
        <a
          href={`https://www.meteora.ag/dlmm/${pos.pool}`}
          target="_blank"
          rel="noreferrer"
      ...
      <td
        className={cn(
          ...
          pos.pnlPct >= 0 ? "text-green-400" : "text-red-400",
        )}
      >
        {fmtPct(pos.pnlPct)}
      </td>
      ...
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="flex items-center gap-1">
                ...
                <a
                  href={`https://solscan.io/account/${owner.owner}`}
                  target="_blank"
                  rel="noreferrer"
              ...
              <div className="mt-1 flex flex-wrap gap-1.5">
                <span
                  className={cn(
                    ...
                    owner.preferredStrategy === "spot"
                      ? "bg-blue-900/50 text-blue-300"
                      : owner.preferredStrategy === "bid_ask"
                        ? "bg-purple-900/50 text-purple-300"
                        : "bg-zinc-800 text-zinc-400",
                  )}
                >
                ...
                {owner.preferredRangeStyle && (
                  <span className="rounded bg-zinc-800 px-1.5 py-px text-xs text-zinc-400">
                    {owner.preferredRangeStyle}
                  </span>
                )}
              </div>
          ...
          <div className="text-right shrink-0">
            <div
              className={cn(
                "text-sm font-bold",
                owner.avgPnlPct >= 0 ? "text-green-400" : "text-red-400",
              )}
            >
        ...
        <div className="mt-3 grid grid-cols-3 gap-3 text-xs">
          <div>
            ...
            <br />
            <span className="text-zinc-300">{fmtHours(owner.avgHoldHours)}</span>
          </div>
          <div>
            <span className="text-zinc-600">Avg fee</span>
            <br />
            <span className="text-zinc-300">{owner.avgFeePercent.toFixed(2)}%</span>
          </div>
          <div>
            <span className="text-zinc-600">Avg width</span>
            <br />
            <span className="text-zinc-300">{owner.avgWidthBins?.toFixed(0) ?? "—"} bins</span>
          </div>
        </div>
        ...
        {expanded && owner.topPositions?.length > 0 && (
          <div className="mt-2 overflow-x-auto rounded border border-zinc-800 max-w-full">
            <table className="w-full text-xs">
              ...
              <tbody>
                {owner.topPositions.map((pos) => (
                  <PositionRow key={pos.position} pos={pos} />
                ))}
              </tbody>
            </table>
          </div>
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        ...
        <p className="text-xs text-zinc-600">
          Enter a pool address to see each LPer&apos;s preferred strategy, range style, and full
          position history.
        </p>
        <div className="flex gap-2">
          ...
          />
          <Button
            onClick={() => poolInput.trim() && setPool(poolInput.trim())}
          ...
          <div className="inline-flex items-center gap-2 rounded-lg border border-zinc-700 bg-zinc-900/60 px-3 py-2">
            ...
            <span
              className={cn(
                ...
                suggestedStyle.strategy === "spot"
                  ? "bg-blue-900/50 text-blue-300"
                  : suggestedStyle.strategy === "bid_ask"
                    ? "bg-purple-900/50 text-purple-300"
        ...
        {historicalOwners.length > 0 && (
          <div className="space-y-3">
            {historicalOwners.map((owner, i) => (
              <WalletCard key={owner.owner} owner={owner} rank={i + 1} />
            ))}
          </div>
        )}
      </div>
      ...
      <div className="space-y-4">
        <h3 className="text-sm font-medium text-zinc-300">
          HiveMind Collective — Tag Win/Loss ({lessons.length} lessons from network)
        </h3>

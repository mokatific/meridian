  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        <div>
          <a
            href={`https://www.meteora.ag/dlmm/${poolAddress}`}
            target="_blank"
            rel="noreferrer"
          ...
          {pos.isOutOfRange && (
            <span className="ml-2 rounded bg-amber-900/40 px-1.5 text-amber-400">OOR</span>
          )}
          {pos.isClosed && (
            <span className="ml-2 rounded bg-zinc-800 px-1.5 text-zinc-500">closed</span>
          )}
        </div>
        <div
          className={cn(
            "text-sm font-bold",
            pos.pnlPctChange >= 0 ? "text-green-400" : "text-red-400",
          )}
        >
      ...
      <div className="mt-2 grid grid-cols-3 gap-2">
        <div>
          <span className="text-zinc-600">PnL USD</span>
          <br />
          <span className={pos.pnlUsd >= 0 ? "text-green-400" : "text-red-400"}>
            {fmtUsd(pos.pnlUsd)}
          </span>
        </div>
        <div>
        ...
        {pos.allTimeFees?.total?.usd != null && (
          <div>
            <span className="text-zinc-600">Fees earned</span>
            <br />
            <span className="text-zinc-300">{fmtUsd(pos.allTimeFees.total.usd)}</span>
          </div>
        )}
        {pos.allTimeDeposits?.total?.usd != null && (
          <div>
            ...
            <br />
            <span className="text-zinc-300">{fmtUsd(pos.allTimeDeposits.total.usd)}</span>
          </div>
        )}
        {pos.feePerTvl24h != null && (
          <div>
            <span className="text-zinc-600">Fee/TVL 24h</span>
            <br />
            <span className="text-zinc-300">{(pos.feePerTvl24h * 100).toFixed(2)}%</span>
          </div>
        )}
      </div>
      {created && (
        <div className="mt-1 text-zinc-600">
          ...
          {closed ? ` → ${closed.toLocaleString()}` : " (open)"}
        </div>
      )}
    </div>
  );
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div>
            <div className="flex items-center gap-2">
              ...
              <a
                href={`https://birdeye.so/profile/${wallet}?chain=solana`}
                target="_blank"
                rel="noreferrer"
                className="text-xs text-zinc-600 hover:text-zinc-400"
              >
            ...
            <div className="mt-1 text-xs text-zinc-600">
              {data.lastActivity
                ? `Last active: ${new Date(data.lastActivity * 1000).toLocaleString()}`
                : "No recent activity"}
            </div>
          </div>
                ...
                <div key={sig.signature} className="flex items-center gap-2 text-xs">
                  <span
                    className={cn(
                      ...
                      sig.err ? "bg-red-500" : "bg-green-500",
                    )}
                  />
                  <a
  ...
  const loadPositions = async () => {
    ...
    try {
      const res = await fetch(
        `/api/meteora/positions?pool=${encodeURIComponent(pool.trim())}&user=${encodeURIComponent(wallet)}&status=${posStatus}&pageSize=50`,
      );
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-2">
          <Input
            ...
            onKeyDown={(e) =>
              e.key === "Enter" && walletInput.trim() && setWallet(walletInput.trim())
            }
      ...
      {wallet && (
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            <Input
              placeholder="Pool address (required)…"
              value={poolInput}
            ...
            <Button
              ...
              disabled={!poolInput.trim() || posLoading}
              className="bg-zinc-700 hover:bg-zinc-600 text-zinc-100"
            >
          ...
          {positions.length > 0 && (
            <div className="space-y-2 max-h-[60vh] overflow-y-auto pr-1">
              ...
              {positions.map((pos) => (
                <PositionCard key={pos.positionAddress} pos={pos} poolAddress={pool} />
              ))}
            </div>
          )}
          ...
          {!posLoading && positions.length === 0 && pool && (
            <p className="text-xs text-zinc-600">
              No {posStatus} positions found for this wallet/pool combination.
            </p>
          )}
        </div>
      )}
      ...
      {!wallet && (
        <p className="py-8 text-center text-sm text-zinc-600">
          Enter a wallet address to begin. Try any top LPer address from the Pool Leaderboard or
          Strategy Decoder.
        </p>
      )}
    </div>

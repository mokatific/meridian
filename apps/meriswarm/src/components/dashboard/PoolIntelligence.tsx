...
function OrganicBadge({ score, label }: { score?: number; label?: string }) {
  ...
  return (
    <span className={cn("rounded border px-1.5 py-px text-xs", color)}>{label ?? `${score}%`}</span>
  );
}
...
function RiskBadge({ token }: { token?: MeteoraPool["token_x"] }) {
  ...
  const label = warnings.length
    ...
    : token.has_freeze_authority
      ? "freeze auth"
      : "mint auth";
  return (
    <span
      className="rounded border border-red-800/50 bg-red-950/30 px-1.5 py-px text-xs text-red-400"
  ...
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-900/60 p-3">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {pool.fee_pct > 0 && <span className="text-xs text-zinc-600">{pool.fee_pct}% fee</span>}
            {pool.launchpad && (
              <span className="rounded bg-zinc-800 px-1.5 py-px text-xs text-zinc-500">
                {pool.launchpad}
              </span>
            )}
          </div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div className="text-right">
            <div
              className={cn(
                "text-sm font-bold",
                pool.pool_price_change_pct != null
                  ? pool.pool_price_change_pct >= 0
                    ? "text-green-400"
                    : "text-red-400"
                  : "text-zinc-400",
              )}
      ...
      <div className="mt-2 grid grid-cols-4 gap-2 text-xs">
        <div>
          ...
          {pool.volume_change_pct != null && (
            <span
              className={cn(
                "ml-1",
                pool.volume_change_pct >= 0 ? "text-green-500" : "text-red-500",
              )}
            >
        ...
        <div>
          <span className="text-zinc-600">Active TVL</span>
          <br />
          <span className="text-zinc-300">{fmtUsd(pool.active_tvl ?? 0)}</span>
        </div>
        <div>
          <span className="text-zinc-600">Fee/aTVL</span>
          <br />
          <span className="text-zinc-300">
            {pool.fee_active_tvl_ratio != null
              ? `${(pool.fee_active_tvl_ratio * 100).toFixed(2)}%`
              : "—"}
          </span>
        </div>
        <div>
      ...
      {expanded && (
        <div className="mt-2 grid grid-cols-3 gap-2 text-xs border-t border-zinc-800 pt-2">
          ...
          <div>
            ...
            <br />
            <span className="text-zinc-300">{pool.open_positions ?? "—"}</span>
          </div>
          <div>
            <span className="text-zinc-600">Active positions</span>
            <br />
            <span className="text-zinc-300">
              {pool.active_positions ?? "—"} (
              {pool.active_positions != null && pool.open_positions
                ? `${((pool.active_positions / pool.open_positions) * 100).toFixed(0)}%`
                : "—"}
              )
            </span>
          </div>
          <div>
            <span className="text-zinc-600">Unique LPers</span>
            <br />
            <span className="text-zinc-300">{pool.unique_lps ?? "—"}</span>
          </div>
          <div>
            <span className="text-zinc-600">Unique traders</span>
            <br />
            <span className="text-zinc-300">{pool.unique_traders ?? "—"}</span>
          </div>
          <div>
            <span className="text-zinc-600">Correlation</span>
            <br />
            <span className="text-zinc-300">{pool.correlation?.toFixed(3) ?? "—"}</span>
          </div>
          {base?.market_cap && (
            <div>
              <span className="text-zinc-600">Base MCap</span>
              <br />
              <span className="text-zinc-300">{fmtUsd(base.market_cap)}</span>
            </div>
          )}
          {base?.holders && (
            <div>
              ...
              <br />
              <span className="text-zinc-300">{base.holders.toLocaleString()}</span>
            </div>
          )}
          <div>
            <a
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="ml-auto text-xs text-zinc-600">
          {pools.length > 0 ? `${pools.length} pools` : ""}

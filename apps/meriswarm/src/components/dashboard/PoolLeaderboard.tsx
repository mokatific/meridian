  ...
  return (
    <div className="mt-2 rounded-md bg-zinc-800/60 p-3 text-xs text-zinc-400 grid grid-cols-3 gap-2">
      <div>
        ...
        <br />
        <span className="text-zinc-200">{fmtUsd(entry.pnlUsd)}</span>
      </div>
      <div>
        <span className="text-zinc-600">Win Rate</span>
        <br />
        <span className="text-zinc-200">{entry.winRatePct.toFixed(1)}%</span>
      </div>
      <div>
        <span className="text-zinc-600">Avg Hold</span>
        <br />
        <span className="text-zinc-200">{fmtHours(entry.avgAgeHours)}</span>
      </div>
    </div>
  );
}
    ...
    <div className="space-y-4">
      ...
      {topData?.overview && (
        <div className="flex flex-wrap gap-3 text-xs text-zinc-500">
          ...
          {studyData && (
            <span>
              {studyData.activePositionCount} active positions · {studyData.ownerCount} LPers
            </span>
          )}
        </div>
      )}
      ...
      {!pool && (
        <p className="py-8 text-center text-sm text-zinc-600">
          Enter a pool address to load the leaderboard.
        </p>
      )}
                    ...
                    >
                      ...
                      <td
                        className={cn(
                          ...
                          lp.totalPnlUsd >= 0 ? "text-green-400" : "text-red-400",
                        )}
                      >
                      ...
                      <td
                        className={cn(
                          "py-2.5 px-2 text-right",
                          lp.roiPct >= 0 ? "text-green-400" : "text-red-400",
                        )}
                      >
                        {fmtPct(lp.roiPct)}
                      </td>
                      <td className="py-2.5 px-2 text-right text-zinc-300">
                        {lp.winRatePct.toFixed(1)}%
                      </td>
                      <td className="py-2.5 px-2 text-right text-zinc-400">
                        {fmtHours(lp.avgAgeHours)}
                      </td>
                      <td className="py-2.5 px-2 text-right text-zinc-400">
                        {lp.feePercent.toFixed(2)}%
                      </td>

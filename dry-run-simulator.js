...
function loadVirtualLog() {
  ...
  try {
    return JSON.parse(fs.readFileSync(VIRTUAL_LOG, "utf8"));
  } catch {
    return { positions: [] };
  }
}
...
export async function registerVirtualPosition(deployResult, poolCandidate, deployAmount) {
  ...
  log(
    "simulator",
    `Virtual position registered: ${positionId} | ${virtualPos.pool_name} | ${deployAmount} SOL | initial_price=${virtualPos.initial_price}`,
  );
  return positionId;
}
...
export async function evaluateVirtualPositions() {
  const state = loadState();
  const virtualPositions = Object.values(state.positions).filter((p) => p.virtual && !p.closed);
...
async function _closeVirtualPosition(pos, evaluation) {
  ...
  log(
    "simulator",
    `Virtual close: ${pos.pool_name} | ${evaluation.reason} | PnL: ${evaluation.pnl_pct}%`,
  );
  ...
  if (isLikelyRug && pos.base_mint) {
    try {
      ...
      log(
        "simulator",
        `Auto-blacklisted ${pos.base_mint} (fast stop loss in ${evaluation.minutes_held}m)`,
      );
    } catch {
      /* non-critical */
    }
...
async function _optimizeConfig(closedPositions) {
  try {
    ...
    if (suggestions.length > 0) {
      ...
      let lessonsData;
      try {
        lessonsData = JSON.parse(fs.readFileSync(LESSONS_FILE, "utf8"));
      } catch {
        lessonsData = { lessons: [] };
      }
      const existingIdx = lessonsData.lessons.findIndex(
        (l) => l.rule && l.rule.startsWith("[CONFIG-OPTIMIZER"),
      );
      if (existingIdx >= 0) {
        ...
        log(
          "simulator",
          `Config optimizer: updated existing lesson (${suggestions.length} suggestion(s))`,
        );
      } else {
  ...
  if (closed.length > 0) {
    ...
    closed
      ...
      .forEach((p) => {
        ...
        lines.push(
          `• ${p.pool_name}: ${sign}${p.final_pnl_pct?.toFixed(1)}% (${p.close_reason?.split(":")[0]})`,
        );
      });
  }
